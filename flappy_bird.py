...
def resetGame():
    global playerScore, pipeGroup, bird
    playerScore = 0
    pipeGroup.empty()
    bird.rect.x = 200
...
class FlappyBird(pygame.sprite.Sprite):
    def __init__(self, x_coordinate, y_coordinate):
        ...
        for i in range(1, 4):
            image = pygame.image.load(f'G:\\College\\MCA\\Sem 1\\Project\\Project\\images\\bird_{i}.png')
    ...
    def update(self):
        global birdFlying, gameOver, scrollSpeed
        if birdFlying:
            self.velocity += 0.5
...
class Pipe(pygame.sprite.Sprite):
    def __init__(self, x_coordinate, y_coordinate, position):
        super().__init__()
        self.image = pygame.image.load('G:\\College\\MCA\\Sem 1\\Project\\Project\\images\\pipe.png')
        self.rect = self.image.get_rect()
        if position == 1:
...
def main():
    global display_screen, birdFlying, gameOver, scrollSpeed, pipeGap, playerScore, pipeGroup, bird, baseScroll, SCREEN_WIDTH, SCREEN_HEIGHT
    pygame.init()
    ...
    # Load images
    background = pygame.image.load('G:\\College\\MCA\\Sem 1\\Project\\Project\\images\\background.png')
    base = pygame.image.load('G:\\College\\MCA\\Sem 1\\Project\\Project\\images\\base.png')
    button = pygame.image.load('G:\\College\\MCA\\Sem 1\\Project\\Project\\images\\restart.png')
    ...
    while game_run:
        ...
        if len(pipeGroup) > 0:
            if (birdGroup.sprites()[0].rect.left > pipeGroup.sprites()[0].rect.left and
                birdGroup.sprites()[0].rect.left < pipeGroup.sprites()[0].rect.right and
                not passPipe):
                passPipe = True
            if passPipe and birdGroup.sprites()[0].rect.left > pipeGroup.sprites()[0].rect.right:
                playerScore += 1
                passPipe = False
        ...
        if gameOver and restartButton.draw():
            gameOver = False
